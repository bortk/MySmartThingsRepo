 ...
 *
 ...
 https://community.hubitat.com/t/release-xiaomi-aqara-opple-drivers-with-presence/41537
 */
...
metadata {
    ...
    preferences {
        ...
        Integer physicalButtons = 6 //getDataValue('physicalButtons') != null ? getDataValue('physicalButtons').toInteger() :  0
...
ArrayList<String> refreshActual(String newModelToSet) {
    ...
    newModelToSet = newModelToSet != null ? newModelToSet : 'lumi.remote.b686opcn01'
    logging("refreshActual() 2 model=$newModelToSet", 1)
...
void initialize() {
    ...
    unschedule()
     log.info "B: initialize "
      log.info "B: call refreshActual(null) "
    refreshActual(null)
}

void installed() {
      log.info "B: installed "
...
void updated() {
     log.info "B: updated "
    logging("updated()", 100)
    log.info "B: call createAllButtonChildren "
...
String setCleanModelNameWithAcceptedModels(String newModelToSet=null) {
     log.info "B: setCleanModelNameWithAcceptedModels "
    List<String> acceptedModels=[
    ...
    return setCleanModelName(newModelToSet, acceptedModels)
}

boolean isSwitchModel(String model=null) {
    return false;
// model = model != null ? model : getDataValue('model')
// switch(model) {
//     case "lumi.sensor_86sw1lu":
//     case "lumi.sensor_86sw1":
//     case "lumi.sensor_86sw2Un":
...
boolean isOppleModel(String model=null) {
    log.info "isOppleModel 1 model: ${model} "
    model = model != null ? model : getDataValue('model')
    log.info "isOppleModel 2 model: ${model} "
    switch(model) {
        case "lumi.remote.b286opcn01":
        case "lumi.remote.b486opcn01":
        case "lumi.remote.b686opcn01":
            log.info "isOppleModel model: ${model} TRUE"
            return true
            break
        default:
            log.info "isOppleModel model: ${model} FALSE"
            return false
    }
}
...
ArrayList<String> parse(String description) {
    ...
    String cModel = getDataValue('model')
...
void parseButtonEvent(Map msgMap) {
    ...
    Integer physicalButtons = getDataValue("physicalButtons") != null ? getDataValue("physicalButtons").toInteger() : 1
        ...
        if(btn == 0 || btn == 16) {
            ...
            if(millisHeld > millisForHoldLong) {
                if(useTimerForHeld != true) {
                    ...
                    String model = model != null ? model : getDataValue('model')
...
ArrayList<String> zigbeeReadAttribute(Integer cluster, Integer attributeId, Map additionalParams = [:], int delay = 205) {
    ...
    logging("zigbeeReadAttribute()")
    logging("zigbeeReadAttribute cluster=$cluster attributeId=$attributeId clustadditionalParamser=$additionalParams ")
    ArrayList<String> cmd = zigbee.readAttribute(cluster, attributeId, additionalParams)
    cmd[0] = cmd[0].replace('0xnull', '0x01')
...
String setCleanModelName(String newModelToSet=null, List<String> acceptedModels=null) {
    log.info "B: setCleanModelName "
    String model = newModelToSet != null ? newModelToSet : getDataValue('model')
    ...
    log.info "B: setCleanModelName model = $model"
    return newModel
}
...
void createButtonChildDevice(String id1, String id2, String type) {
    logging("createButtonChildDevice id1 = $id1 id2 = $id2 type = $type")
    String driver = null
    String name = null
    String id = null
    if(useDimmerChildSet(type) == true) {
        ...
        logging("createButtonChildDevice Virtual Momentary Switch")
        ...
        logging("createButtonChildDevice Virtual Button")
        ...
        logging("createButtonChildDevice Virtual Switch")
        driver = "Generic Component Switch"
        name = "Virtual Switch"
        id = id1
        if(id2 != null) {
    ...
    try {
        ...
        physicalgraph.app.DeviceWrapper cd = addChildDevice("smartthings", "Child Button", "$device.id-$id", [name: "$name $id", label: "$name $id", isComponent: false])
        logging("CREATED device with type $type and id $device.id-$id", 100)
        if(useDimmerChildSet(type) == true) {
            logging("DEBUG useDimmerChildSet")
            ...
            logging("DEBUG useVirtualButtonChildSet")
            ...
            logging("DEBUG else switch")
            cd.parse([[name: "switch", value: 'off', isStateChange: true, descriptionText: "Switch Initialized as OFF"]])
        }
    } 
    catch (physicalgraph.app.exception.UnknownDeviceTypeException e) {
        logging("ERROR UnknownDeviceTypeException")
        ...
        log.error "Boris: Exception occured: '$e'"
    } 
}
...
// Boris
// physicalgraph.app.DeviceWrapper cd = getChildDevice(deviceID)
...
// Boris
// physicalgraph.app.DeviceWrapper cd = getChildDevice(data["deviceID"])
...
void componentRefresh(physicalgraph.app.DeviceWrapper cd) {
    logging("componentRefresh() from $cd.deviceNetworkId", 1)
}

void componentOn(physicalgraph.app.DeviceWrapper cd) {
...
void componentOff(physicalgraph.app.DeviceWrapper cd) {
...
void componentStopLevelChange(physicalgraph.app.DeviceWrapper cd) {
    logging("componentStopLevelChange() from $cd.deviceNetworkId", 1)
    unschedule("runLevelChange_${cd.deviceNetworkId.split("-")[1]}")
}

void componentStartLevelChange(physicalgraph.app.DeviceWrapper cd, String direction) {
...
void componentSetLevel(physicalgraph.app.DeviceWrapper cd, BigDecimal level) {
    componentSetLevel(cd, level, null)
}

void componentSetLevel(physicalgraph.app.DeviceWrapper cd, BigDecimal level, BigDecimal duration) {
