 ...
 */
	definition (name: "Boris Xiaomi Aqara Wireless SwitchFan", namespace: "bortk-dev", author: "bortk", minHubCoreVersion: "000.022.0002", ocfDeviceType: "oic.d.fan" ) {
		capability "Battery"
		capability "Sensor"
		capability "Button"
...
def setLevel(value, rate = null) {
	displayInfoLog(": setLevel ${value}")
    displayDebugLog(": setLevel ${value}")
}

def setFanSpeed(speed) {
	displayInfoLog(": setFanSpeed ${speed}")    
    displayDebugLog(": setFanSpeed !! ${speed}")
	def result = mapButtonEvent(1, 3)
	displayDebugLog(": setFanSpeed Sending event $result")
	sendEvent(result)
    //sendEvent(name: "button", value: "pushed", data: [buttonNumber:2], descriptionText: "Virtual button was pushed (Button 2 pushed) descriptionText ${speed}", isStateChange: true)
}
def lowSpeed() {
	displayDebugLog(": lowSpeed! ")
	setFanSpeed("1")
}

def medSpeed() {
displayDebugLog(": medSpeed! ")
	setFanSpeed("2")
}

def medHighSpeed() {
displayDebugLog(": medHighSpeed! ")
	setFanSpeed("3")
}

def highSpeed() {
displayDebugLog(": highSpeed! ")
	setFanSpeed("4")
}

def breeze() {
displayDebugLog(": breeze! ")
	setFanSpeed("6")
}
...
def on() {
    ...
    def result = mapButtonEvent(1, 2)
	displayDebugLog(": on sending event $result")
	sendEvent(result)
}
...
def off() {
    ...
    def result = mapButtonEvent(1, 1)
	displayDebugLog(": off sending event $result")
	sendEvent(result)
}
def low() {
	setLevel( 25)
