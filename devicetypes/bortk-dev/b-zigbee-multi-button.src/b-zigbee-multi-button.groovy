...
metadata {
    definition (name: 'B Zigbee Multi Button', namespace: 'bortk-dev', author: 'SmartThings', mcdSync: true, ocfDeviceType: 'x.com.st.d.remotecontroller') {
        capability 'Actuator'
        capability 'Battery'
        capability 'Button'
...
def parse(String description) {
    log.debug "1 Description ${description} "
...
private addChildButtons(numberOfButtons) {
    for (def endpoint : 1..numberOfButtons) {
        try {
            ...
            def child = addChildDevice('B Child Button', childDni, device.getHub().getId(), [
                    completedSetup: true,
